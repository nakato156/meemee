...
(function(){
    ...
    mainPosts.addEventListener('click', (e)=>{
        const element = e.target
        const btn = element.closest('button')
        if(btn){
            const post = element.closest('.post')
            if(btn.classList.contains('like')){
                toggleLike(post.id, btn)
            }else {
                const shareData = {
                    title: "MeeMee",
                    text: "Publicacion",
                    url: `${location.href}${post.id}`,
                  };
...
function showPosts(posts, mainPosts){
    ...
    posts.forEach(post => {
        ...
        temp += `<div class="post grid gap-4 mx-auto" id="${post._id}">
            <div class="border-0 pb-3" data-v0-t="card">
                ...
                <div class="items-center p-2 grid gap-2">
                    <div class="flex items-center w-full">
                        ...
                        <button
                            class="share inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 hover:bg-accent hover:text-accent-foreground h-10 w-10">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                    ...
                    <span class="flex"><p id="likeCount${post._id}" class="px-2">${post.likeCount}</p> Me gusta</span>
